...
const Input = React.forwardRef<HTMLInputElement, InputProps>(
  ({ className, type, endIcon, onFocus, onBlur, ...props }, ref) => {
    ...
    return (
      <div
        className={cn(
          "flex items-center justify-between bg-transparent rounded-md overflow-hidden border-2 w-full transition-all duration-300",
          focus ? "border-primary" : "  border-black"
        )}
      >
        <input
          type={type}
